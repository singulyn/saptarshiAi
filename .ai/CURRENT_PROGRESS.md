...
- Lightweight in-memory AccessControl role, permission, user-role, and role-permission services for the Admin MVC workflow.
- Real Admin MVC Roles and Permissions pages with AJAX list/form flows.
- Role Permissions drawer and User Roles drawer.
- Direct sidebar entries for Users, Roles, and Permissions without dynamic permission gating.
- Users, Roles, and Permissions pages now reuse the Organization module page shell, filters, table wrappers, action buttons, and pagination styling.
...
- AccessControl in-memory repositories are registered by `SaptariX.Platform.AccessControl`.

Needs testing:

- SQL scripts against a local SQL Server container.
- New Users stored procedures against SQL Server.
- Browser verification for `/Roles`, `/Permissions`, user-role drawer, role-permission drawer, and direct sidebar entries.
